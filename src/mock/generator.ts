...
function randomDate(index?: number, total?: number): string {
  ...
  if (index !== undefined && total !== undefined && total > 0) {
    const baseOffset = (index / total) * daysBack;
    const interval = daysBack / total;
    const randomVariation = (Math.random() - 0.5) * 0.2 * interval;
    const daysAgo = baseOffset + randomVariation;
...
export function generateMockReports(count: number): Report[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const createdAt = randomDate(i, count);
    ...
    const report: Report = {
      ...
      escalation: {
        ...
        escalated_at:
          Math.random() > 0.8 ? randomDateAfter(createdDate, 7) : null,
      },
